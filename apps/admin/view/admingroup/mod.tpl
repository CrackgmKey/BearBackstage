...
$(document).ready(function(){
	...
	$("#mod").click(function(){
		$("#form1").attr("action","{:url('admin/admingroup/mod')}");
		$("#form1").submit();
	})
...
<div class="main" id="main">
	...
	<div class="add-box">
		<ul>
			<li><div class="title">名称</div><div class="input-box"><input name="name" type="text" class="input" value="{$data.name}"/></div><div class="tip">*</div><div class="clear"></div></li>
			...
			{volist name="class_datas" id="class_data"}
			  ...
			  {volist name="node_datas" id="node_data"}
				{if condition="$class_data.id==$node_data.class_id"}
					<div class="auth-body">
					<input type="checkbox" class="node_{$class_data.id}" name="auth[]" value="{$node_data.id}"
						
						{volist name="auths" id="auth" }
							{if condition="$node_data.id==$auth"} checked="checked" {/if}
						{/volist}
						/>&nbsp;{$node_data.name}
					</div>
				{/if}
