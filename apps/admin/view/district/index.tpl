...
$(document).ready(function(){
	...
	$("#update").click(function(){
		$("#form1").attr("action","{:url('admin/district/batchSort')}");
		$("#form1").submit();
	})
	...
	$("#dels").click(function(){
		...
		layui.use(['layer', 'form'], function(){
			layer.confirm('你确定要删除吗?', {icon: 3, title:'提示'}, function(index){
			 $("#form1").attr("action","{:url('admin/district/del')}");
			 $("#form1").submit();
			  layer.close(index);
			});
		});
	...
	$("#add").click(function(){
		window.location.href="{:url('admin/district/add')}";
	})
...
<div class="main" id="main">
	<div class="position link grey_l"><a href="{:url('admin/index/index')}" target="_top">首页</a>&nbsp;>&nbsp;内容管理&nbsp;>&nbsp;地区管理</div>
	<div class="list-box">
		<div class="title">
			<ul>
		...
		<div class="list" id="list">
				...
				<ul>
					...
					<li style="width:55%"><div class="left" style="width:100px;">{$data.district_name}</div>{if condition="$data.is_sub"}<div class="up-icon slide" id="{$data.id}" title="下拉子导航"></div>{/if}</li>
				...
				<div class="sub sub_{$data.id}">
					{volist name="data.sub" id="d"}
						<ul >
							...
							<li style="width:55%;"><div class="ntree-icon"></div><div class="left name">{$d.district_name}</div></li>
