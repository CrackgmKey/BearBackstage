<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:46:"D:\phpStudy\WWW\yg/apps/admin\view\nav\mod.tpl";i:1501773004;s:48:"D:\phpStudy\WWW\yg\apps\admin\view\index\top.tpl";i:1550568981;}*/ ?>
...
$(document).ready(function(){
	...
	$("#mod").click(function(){
		$("#form1").attr("action","<?php echo url('admin/nav/mod'); ?>");
		$("#form1").submit();
	})
...
<div class="main" id="main">
	<div class="position link grey_l"><a href="<?php echo url('admin/index/index'); ?>" target="_top">首页</a>&nbsp;>&nbsp;内容管理&nbsp;>&nbsp;编辑导航</div>
	
	<div class="add-box">
		<ul>
			<li><div class="title">导航名称</div><div class="input-box"><input name="name" type="text" class="input" value="<?php echo $data['name']; ?>"/></div><div class="tip">*</div><div class="clear"></div></li>
			<li>
				<div class="title">所属栏目</div>
				<div class="input-box-s" style="z-index:100"><span class="select-title"><?php echo $data['currentName']; ?></span>
					<div class="select-box"  id="nav-list">
						<ul>
							<input type="hidden" name="pid" value="<<?php echo $data['pid']; ?>>">
							<li id="0">顶级栏目</li>
							 <?php if(is_array($navdatas) || $navdatas instanceof \think\Collection || $navdatas instanceof \think\Paginator): $i = 0; $__LIST__ = $navdatas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?>
								<li id="<?php echo $d['id']; ?>" mid="<?php echo $d['module_id']; ?>"><?php echo $d['name']; ?></li>
			...
			<li id="module_list" >
				...
				<div class="input-box-s" style="z-index:99"><span class="select-title"><?php echo $data['module_name']; ?></span>
					<div class="select-box" id="module-list">
						<ul>
							<input type="hidden" name="module_id" id="module_id" value="<?php echo $data['module_id']; ?>">
							 <?php if(is_array($moduledatas) || $moduledatas instanceof \think\Collection || $moduledatas instanceof \think\Paginator): $i = 0; $__LIST__ = $moduledatas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?>
								<li id="<?php echo $d['id']; ?>"><?php echo $d['name']; ?></li>
							 <?php endforeach; endif; else: echo "" ;endif; ?>
						</ul>
					</div>
				</div>
			...
			<li><div class="title">栏目排序</div><div class="input-box"><input name="sort" type="text" class="input" value="<?php echo $data['sort']; ?>"/></div><div class="tip">*</div><div class="clear"></div></li>
			<li>
				<div class="title">是否显示</div>
				<div class="radio-box1">
					<div class="<?php if($data['display']==1): ?>radio-s<?php else: ?>radio-n<?php endif; ?>  radio" id="1"></div><div class="radio-t">显示</div>
					<div class="<?php if($data['display']==0): ?>radio-s<?php else: ?>radio-n<?php endif; ?>  radio" id="0" style="margin-left:30px;"></div><div class="radio-t">隐藏</div>
					<input type="hidden" name="display" value="<?php echo $data['display']; ?>">
				</div>
				<div class="clear"></div>
			</li>
		</ul>
